...
\begin{frame}
	\frametitle{Structure of the PhD project}
	\begin{itemize}
		...
		\alert<4>{Maximum-likelihood estimation of xylem vessel lengths}\visible<5>{: \alert<5>{\textbf{Not in the focus of this presentation!}}
		}
    \end{itemize}	
\end{frame}
...
\begin{frame}
	\frametitle{Water transport in plants}
		\begin{changemargin}{-2em}{-2em}
			\begin{minipage}{0.52 \paperwidth}
				\includegraphics[width = \tw]{figures/intro_01_stroock.png}\\
				\centering{\source{Image source: Stroock et al., 2014}}
			\end{minipage}
			\begin{minipage}{0.4 \paperwidth}
			\begin{itemize}				
				\item<1-> Driving force: \alert<1>{gradient in water potentials}
				\item<2-> Continuous water columns held together by \alert<2>{\textit{cohesion-tension-mechanism}}
				\item<visible@3-| alert@3> [\rar]Liquid under tension: \textbf{metastable state}
				\item<visible@4| alert@4> [\rar] If negative pressure too high:\\ \textbf{risk of embolism \&\\ loss of conductance}
				\end{itemize}
			\end{minipage}
		\end{changemargin}
\end{frame}
...
\begin{frame}
	...
	\begin{minipage}{0.5\tw}
	 ...
	 \begin{itemize}
	 	...
	 	\item<+-| alert@+> 4 measurement campaigns of $\pm$ 1 week during rainy season of \alert<3>{2015}
	 	\item<4-| alert@4> 40 trees of 8 species	
	 	\item<5-| alert@5> Measured with the Heat Field Deformation (HFD) method			
	 \end{itemize}			
	\end{minipage}
	\begin{minipage}{0.48\tw}
		\only<1-2>{\includegraphics[width = \tw]{figures/map_02_horizontes.png}} 	
		\only<3-4>{\includegraphics[width = \tw]{figures/Guanacaste2015.jpg}\\
			
			\source{Image source: Instituto Meteorológico Nacional de Costa Rica}} 			
		\only<5>{\centering \includegraphics[width = 0.65\tw]{figures/HFD_01_sensor.JPG}}
	\end{minipage}
\end{frame}
...
\begin{frame}
	...
	\begin{minipage}{0.5\tw}
		...
		\begin{itemize}
			...
			\item<+-| alert@+> Temperature differences between sensors are used to estimate sap flux density at different depths
		\end{itemize}						
	\end{minipage}
	\begin{minipage}{0.48\tw}
			\only<1>{\includegraphics[width = \tw]{figures/nadezhdina_01.png}\\
		\source{Image source: Nadezhdina et al., 2012}}
				\only<2>{\centering \includegraphics[width = 0.65\tw]{figures/HFD_01_sensor.JPG}}
...
\begin{frame}[t]
	...
	\begin{itemize}
		...
		\begin{itemize}			
			\item<2-> \alert<2>{\textbf{First stage of the model:}} Nonlinear relationship between sensor depth and predicted flux density modeled with the \alert<3>{density function of the Weibull distribution}
...
\begin{frame}
	...
	\begin{minipage}{0.48\tw}
		...
		\only<4>{\includegraphics[width = 0.8\tw]{figures/field3.jpg}}
...
\end{frame}
	...
	\begin{frame}[plain]
		...
		\vspace{2em}
		\visible<2>{\Blue{Slides and source code of this presentation:}\\
			\url{https://github.com/r-link/2018-01-25_Presentation_PhD}
			}
	\end{frame}


{
	\usebackgroundtemplate{\includegraphics[height=\paperheight,width=\paperwidth]{figures/thankyou.jpg}}
